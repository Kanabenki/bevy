...
pub fn propagate_transforms(
    ...
    thread_stacks: Local<ThreadLocal<Cell<Vec<(Entity, GlobalTransform, bool)>>>>,
) {
    ...
    root_query.par_iter_mut().for_each(
        |(entity, children, transform, mut global_transform)| {
            ...
            // TODO(kanabenki): update safety comments
            let stack_cell = thread_stacks.get_or_default();
            let mut stack = stack_cell.take();
            ...
            for (child, actual_parent) in parent_query.iter_many(children) {
                ...
                stack.clear();
                stack.push((child, *global_transform, changed));
                while let Some((entity, parent, mut changed)) = stack.pop() {
                    ...
                    for (child, actual_parent) in parent_query.iter_many(children) {
                        ...
                        // entire hierarchy.
                        stack.push((child, global_matrix, changed || actual_parent.is_changed()));
                    }
                }
